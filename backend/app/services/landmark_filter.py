...
def apply_landmark(image: np.ndarray) -> np.ndarray:
    background = _load_background(image.shape[1], image.shape[0])
...
def _opencv_person_mask(image: np.ndarray):
    ...
    try:
        # GrabCut tach nguoi tu vung chu the du doan; khong phu thuoc MediaPipe.
...
def _subject_rect(image: np.ndarray):
    ...
    width = min(w - left - 3, int(w * 0.68))
    height = min(h - top - 3, int(h * 0.9))
